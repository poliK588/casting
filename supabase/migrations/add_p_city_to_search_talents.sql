-- =============================================================================
-- PATCH: Add p_city parameter to search_talents RPC
-- Date: 2026-07-08
-- Summary: 2 lines added — p_city in signature + WHERE filter
...
CREATE OR REPLACE FUNCTION public.search_talents(
  ...
  p_city          text[]  DEFAULT NULL::text[],           -- ← NEW
...
BEGIN
  ...
  WHERE p.role = 'talent'
    ...
    AND (p_city IS NULL OR cardinality(p_city) = 0 OR p.city = ANY(p_city))   -- ← NEW
    ...
    AND (
      (p_date IS NULL AND p_date_from IS NULL)
      OR NOT EXISTS (
    ...
    -- Skills (strict: talent must have ALL selected skills)
    AND (p_skills IS NULL OR cardinality(p_skills) = 0 OR (
      SELECT COUNT(DISTINCT us.skill_id)
      FROM user_skills us
      WHERE us.user_id = p.id AND us.skill_id = ANY(p_skills)
    ) = cardinality(p_skills))

    -- Languages (strict: talent must have ALL selected languages)
    AND (p_languages IS NULL OR cardinality(p_languages) = 0 OR (
      SELECT COUNT(DISTINCT ul.language_id)
      FROM user_languages ul
      WHERE ul.user_id = p.id AND ul.language_id = ANY(p_languages)
    ) = cardinality(p_languages))

    -- Ethnicities (match ANY selected ethnicity)
