  ...
  return (
    <div className="relative" onMouseEnter={() => setShowTip(true)} onMouseLeave={() => setShowTip(false)}>
      ...
      >
        ...
        <span className={`text-sm font-500 whitespace-nowrap overflow-hidden transition-all duration-200 ${expanded ? 'opacity-100 max-w-xs' : 'opacity-0 max-w-0'}`}>
          {item.label}
        </span>
        {item.badge && (
          <span className={`flex-shrink-0 ml-auto min-w-[18px] h-[18px] bg-red-500 text-white text-[9px] font-800 rounded-full flex items-center justify-center px-1 border-2 border-navy-900 ${expanded ? '' : 'absolute top-1.5 right-1.5'}`}>
            {item.badge}
          </span>
        )}
      </button>
      {!expanded && showTip && (
        <div className="nav-tooltip absolute left-[72px] top-1/2 -translate-y-1/2 bg-navy-900 border border-white/10 text-white text-xs font-600 px-2.5 py-1 rounded-lg shadow-lg whitespace-nowrap z-50 transition-opacity"
          style={{opacity:showTip?1:0}}>
          {item.label}
          {item.badge && <span className="ml-1.5 bg-red-500 text-white text-[9px] font-800 rounded-full px-1">{item.badge}</span>}
        </div>
      )}
    </div>
  );
...
export default function Sidebar() {
  ...
  return (
    <aside className={`flex flex-col bg-navy-900 h-full flex-shrink-0 overflow-hidden z-20 transition-all duration-300 ${sidebarExpanded ? 'w-52' : 'w-16'}`}>
      {/* Logo */}
      <div className="flex items-center justify-center h-14 border-b border-white/10 px-3 flex-shrink-0">
        ...
        {sidebarExpanded && <span className="ml-2.5 font-800 text-white text-sm tracking-tight whitespace-nowrap overflow-hidden transition-opacity duration-200" style={{opacity:sidebarExpanded?1:0}}>CastingHub</span>}
      </div>
