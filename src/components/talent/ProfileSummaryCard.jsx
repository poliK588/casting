...
export default function ProfileSummaryCard({ user, editLink }) {
  return (
    <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden lift">
      ...
      <div className="px-[18px] pb-[18px]">
        {user.avatar ? (
          <img
            ...
            className="w-[72px] h-[72px] rounded-full object-cover object-top border-[3px] border-white shadow-md -mt-[36px] relative z-10"
          />
        ) : (
          <div className="w-[72px] h-[72px] rounded-full border-[3px] border-white shadow-md -mt-[36px] relative z-10 bg-slate-200 flex items-center justify-center">
        ...
        <div className="flex items-start justify-between mt-1">
          <div>
            <div className="text-[18px] font-[800] text-slate-900 tracking-[-0.02em] mt-1.5">{user.name}</div>
            <div className="text-[12px] text-slate-500 mt-0.5 flex items-center gap-1.5 flex-wrap">
              <span>{user.age} yrs</span><span className="text-slate-300">&bull;</span>
              <span>{user.location}</span>
            </div>
            <div className="flex items-center gap-1.5 mt-1">
              <span className="text-amber-500 text-[14px] tracking-[-0.5px]">★★★★★</span>
              <span className="text-[11px] font-[700] text-slate-900">{user.rating}</span>
        ...
        <div className="grid grid-cols-3 gap-px bg-slate-100 rounded-[10px] overflow-hidden my-3.5">
          <div className="bg-white p-2.5 text-center">
            <div className="text-[16px] font-[800] text-slate-900">{user.credits}</div>
            <div className="text-[10px] text-slate-400 font-[500] mt-[1px]">Credits</div>
          </div>
          <div className="bg-white p-2.5 text-center">
            <div className="text-[16px] font-[800] text-slate-900">{user.submitted}</div>
            <div className="text-[10px] text-slate-400 font-[500] mt-[1px]">Submitted</div>
          </div>
          <div className="bg-white p-2.5 text-center">
            <div className="text-[16px] font-[800] text-slate-900">${user.rate}</div>
        ...
        {editLink ? (
          <Link to={editLink} className="w-full h-9 border-[1.5px] border-navy-900 rounded-[9px] flex items-center justify-center gap-1.5 text-[12px] font-[700] text-navy-900 cursor-pointer transition-colors hover:bg-navy-50 mt-3.5 mt-auto no-underline">
            <Icon name="pencil" size={13} color="currentColor" />
            Edit Profile
          </Link>
        ) : (
          <button className="w-full h-9 border-[1.5px] border-navy-900 rounded-[9px] flex items-center justify-center gap-1.5 text-[12px] font-[700] text-navy-900 cursor-pointer transition-colors hover:bg-navy-50 mt-3.5 mt-auto">
            <Icon name="pencil" size={13} color="currentColor" />
            Edit Profile
          </button>
        )}
